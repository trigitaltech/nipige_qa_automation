...
export default class UIElementActions {
   ...
   * Uses a web-first assertion to wait (auto-retrying) for the element to become visible,
   * then reads its rendered text via innerText. This handles async (e.g. Angular ng-show)
   * UI updates and fails fast with a descriptive message instead of a bare visibility timeout.
   * Use {@link getTextContentEvenIfHidden} when you intentionally need the text of a hidden node.
   * @param timeoutInSec max time to wait for visibility (defaults to CommonConstants.DEFAULT_TIMEOUT)
   * @returns
   */
  public async getTextContent(timeoutInSec: number = CommonConstants.DEFAULT_TIMEOUT): Promise<string> {
    let content = "";
    await test.step(`Getting text content of ${this.description}`, async () => {
      const element = this.getLocator();
      await expect(element, `${this.description} never became visible (element may be hidden, e.g. ng-hide). `
        + `If validating a post-login state, the underlying action likely did not succeed.`)
        .toBeVisible({ timeout: timeoutInSec * CommonConstants.ONE_THOUSAND });
  ...
  /**
   * Gets the text content WITHOUT requiring the element to be visible.
   * The element only needs to be attached to the DOM. Uses textContent (ignores CSS visibility),
   * so this is appropriate for deliberately reading hidden nodes.
   * @param timeoutInSec max time to wait for the element to attach (defaults to CommonConstants.DEFAULT_TIMEOUT)
   * @returns
   */
  public async getTextContentEvenIfHidden(timeoutInSec: number = CommonConstants.DEFAULT_TIMEOUT): Promise<string> {
    let content = "";
    await test.step(`Getting text content (allow hidden) of ${this.description}`, async () => {
      const element = this.getLocator();
      await element.waitFor({ state: "attached", timeout: timeoutInSec * CommonConstants.ONE_THOUSAND });
